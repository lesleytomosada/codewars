...
class PaginationHelper:
    ...
    def page_item_count(self, page_index):
        ...
        if page_index + 1 > num_pages:
            return -1
        if num_pages > page_index + 1:
            return self.items_per_page
        else:
            length = len(self.collection)
            full_pages = self.items_per_page * (page_index)
            return length - full_pages

    # determines what page an item at the given index is on. Zero based indices
    # this method should return -1 for item_index values that are out of range
    def page_index(self, item_index):
        if (
            not self.collection
            or item_index < 0
            or item_index > len(self.collection) - 1
        ):
            return -1
...
class PaginationHelper2:
    ...
    def page_item_count(self, page_index):
        return (
            min(
                self.items_per_page,
                self._item_count - page_index * self.items_per_page,
            )
    ...
    def page_index(self, item_index):
        return (
            ...
            if 0 <= item_index < self._item_count
            else -1
        )
