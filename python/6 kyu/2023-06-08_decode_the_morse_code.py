...
# capital letters are used. When the message is written in Morse code, a
# single space is used to separate the character codes and 3 spaces are used
...
# unused variable warning for morse_codes because it's no longer used and kept
# only for old solutions.
...
# NASM: a table of pointers to the morsecodes, and a corresponding list of
# ascii symbols
    ...
    for i in range(len(words)):
        ...
        if len(letters[i]) > 0:
            for letter in letters[i]:
                word.append(MORSE_CODE[letter])
        translated_letters.append("".join(word))
