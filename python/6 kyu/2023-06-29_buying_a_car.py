...
def nbMonths(
    ...
    savings = 0
    percent_loss_by_month /= 100
    month = 0

    while start_price_old + savings < start_price_new:
        month += 1
        if month > 1 and month % 2 != 1:
            percent_loss_by_month += 0.005
        start_price_old *= 1 - percent_loss_by_month
        start_price_new *= 1 - percent_loss_by_month
        savings += saving_per_month

    return [month, round(start_price_old + savings - start_price_new)]


def nbMonths2(oldCarPrice, newCarPrice, saving, loss):
    months = 0
    budget = oldCarPrice
