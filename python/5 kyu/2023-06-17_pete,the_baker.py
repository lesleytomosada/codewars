...
# can bake (integer). For simplicity there are no units for the amounts (e.g.
# 1 lb of flour or 200 g of sugar are simply 1 or 200). Ingredients that are
...
def cakes(recipe, available):
    ...
    for item in recipe:
        if item not in available:
            return 0
        num = available[item] // recipe[item]
        cakes = min(cakes, num)
    return cakes
