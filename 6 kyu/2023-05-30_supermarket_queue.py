...
#  integer represents a customer, and its value is the amount of time
...
# P.S. The situation in this kata can be likened to the more-computer-
# science-related idea of a thread pool, with relation to running multiple
#  processes at the same time: https://en.wikipedia.org/wiki/Thread_pool


def queue_time(customers, n):
    length = [0] * n
    for i in customers:
        length[length.index(min(length))] += i
    return max(length)
