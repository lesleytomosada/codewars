...
# foam or gas lost every day (evap_per_day) and the threshold (threshold) in
...
def evaporator(content, evap_per_day, threshold):
    days = 0
    expired = content * (threshold * 0.01)
    while content > expired:
        content = content * (1 - evap_per_day * 0.01)
        days += 1
    return days


def evaporator2(content, evap_per_day, threshold):
    n = 0
    current = 100
